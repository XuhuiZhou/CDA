...
from src.modeling_bert_hansent import BertForDocRelationClassification 
...
MODEL_CLASSES = {
    ...
    "bert-han":(BertConfig, BertForDocRelationClassification, BertTokenizer),
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type != "distilbert":
                inputs["token_type_ids"] = (
                    batch[2] if args.model_type in ["bert", "bert-han", "xlnet", "albert"] else None
                )  # XLM, DistilBERT, RoBERTa, and XLM-RoBERTa don't use segment_ids
            model.encode._init_hidden_state(batch[0].size()[0])
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if args.model_type != "distilbert":
                    inputs["token_type_ids"] = (
                        batch[2] if args.model_type in ["bert", "xlnet", "albert", "bert-han"] else None
                    )  # XLM, DistilBERT, RoBERTa, and XLM-RoBERTa don't use segment_ids
                model.encode._init_hidden_state(batch[0].size()[0])
                outputs = model(**inputs)
